...
int screenSpeed = 3, screenPartSpeed = 3, partX = 0, partY = 0;
bool leftHanded = false, mouseRelative = false, bindKeys = false, screenSwap = false;
...
const char * assocstatus_strings[] = {
	...
	"ACQUIRINGDHCP",	// connected to AP, but getting IP data from DHCP
	"ASSOCIATED",		// Connected!
	"CANNOTCONNECT",	// error in connecting...
};
...
/*void centerText (const char* text, uint8 y) {
	int x = ((TEXT_XMAX - strlen(text))/2);
	setText (x, y);
	iprintf ("%s\n", text);
}*/
...
void virtualDesktopMode() {
	u16 delay = 0;
	bool showkeys = false, showMenu = false, quit = false;
	char keybd, str[16] = "";
	int i, zoomBtn = 0;
//	u8 lastPartX = partX, lastPartY = partY;
...
//	showKeyboard(false);
	...
	do {
		...
		if (screenSwap) {
			updateScreenShot(&screenSmall, screenSpeedData[screenSpeed], SCREENPARTVRAM);
			updateScreenShotPart(&screenPart, screenSpeedData[screenPartSpeed], (u8) partX, (u8) partY, SCREENVRAM);
		} else {
			updateScreenShot(&screenSmall, screenSpeedData[screenSpeed], SCREENVRAM);
		...
		if (showkeys) {
			...
			if (bindKeys) {
				...
				if (customKeys[zoomBtn] == ZOM && (keysHeld()&dsKeys[zoomBtn])) {
					if (!showkeys && touchXY.px > 0 && touchXY.py > 0) {
						partX = touchXY.px-25;
						partY = touchXY.py-20;
					}
				...
				//Swap Screens
				if ((!leftHanded&&(keysDown()&KEY_R))||(leftHanded&&(keysDown()&KEY_L))) {
					swapBuffers (SCREENVRAM, SCREENPARTVRAM, 49152);
					screenSwap = !screenSwap;
				}
				//Move Zoomed
				if (touchXY.px > 0 && touchXY.py > 0 && ((!leftHanded&&(keysHeld()&KEY_L))||(leftHanded&&(keysHeld()&KEY_R)))) {
					partX = touchXY.px-25;
					partY = touchXY.py-20;
				}
				
				//Mouse Commands
				if ((!leftHanded && (keysDown()&KEY_LEFT)) || (leftHanded && (keysDown()&KEY_Y)))
					sendCommand (MOUSECLICKDOWN);
				else if ((!leftHanded && (keysUp()&KEY_LEFT)) || (leftHanded && (keysUp()&KEY_Y)))
					sendCommand (MOUSECLICKUP);
				if ((!leftHanded && (keysDown()&KEY_RIGHT)) || (leftHanded && (keysDown()&KEY_A)))
					sendCommand (MOUSERCLICKDOWN);
				else if ((!leftHanded && (keysUp()&KEY_RIGHT)) || (leftHanded && (keysUp()&KEY_A)))
					sendCommand (MOUSERCLICKUP);
				if ((!leftHanded && (keysDown()&KEY_UP)) || (leftHanded && (keysDown()&KEY_X)))
					sendCommand (MOUSEMCLICKDOWN);
				else if ((!leftHanded && (keysUp()&KEY_UP)) || (leftHanded && (keysUp()&KEY_X)))
					sendCommand (MOUSEMCLICKUP);

				//Scroll top view
				if ((!leftHanded&&(keysHeld()&KEY_Y))||(leftHanded&&(keysHeld()&KEY_LEFT)))
					partX--;
				if ((!leftHanded&&(keysHeld()&KEY_A))||(leftHanded&&(keysHeld()&KEY_RIGHT)))
					partX++;
				if ((!leftHanded&&(keysHeld()&KEY_X))||(leftHanded&&(keysHeld()&KEY_UP)))
					partY--;
				if ((!leftHanded&&(keysHeld()&KEY_B))||(leftHanded&&(keysHeld()&KEY_DOWN)))
					partY++;
			}
		}
...
//		if (lastPartX != partX || lastPartY != partY)
//			sendMouseZoomStart((u8) partX, (u8) partY);
...
//		lastPartY = partY;
		...
		if (showMenu) {
			...
			if (topLeftBtn->getState()==BTN_ACTIVATED) {
				...
				screenPartSpeed++;
				if(screenPartSpeed > 5) screenPartSpeed = 0;
				...
				screenPartSpeed--;
				if(screenPartSpeed < 0) screenPartSpeed = 5;
				topSpeedLbl->setTextCenter((char*)screenSpeedString[screenPartSpeed]);
		...
		//Show/hide keyboard
		if (keysDown()&KEY_SELECT && !showMenu) {
			showkeys = !showkeys;
			showKeyboard(showkeys);
		}
		if (keysDown()&KEY_START || (showMenu && OKButton->getState()==BTN_ACTIVATED)) {
			showMenu = !showMenu;
			if (showMenu) {
...
void sendTouchUDP() {
	...
	CWindow *passWnd, *connWnd, *mainWnd;
	CLabel *passLbl, *connLbl;
	CSelector *selDesktop, *selCustomize, *selGamePad, *selDisconnect;
	...
	do {
		...
		while ((keybd = processKeyboard(&str[0], 15, 0) != RET) && passWnd->getState()!=WND_OK) {
			swiWaitForVBlank();
			sGUI->processInput();
			passLbl->setText(str);
...
//	mGUI->destroyWindow(connWnd);
	...
	do {
		...
		if(selDesktop->getState()==SEL_ACTIVATED)
			sel = 1;
		else if(selCustomize->getState()==SEL_ACTIVATED)
			sel = 2;
		else if(selGamePad->getState()==SEL_ACTIVATED)
			sel = 3;
		else if(selDisconnect->getState()==SEL_ACTIVATED)
			sel = 4;
...
bool enterIP(unsigned long *destip, char *name) {
	...
	} while(IPWindow->getState()!=WND_OK && OKButton->getState()!=BTN_ACTIVATED);
...
//	showKeyboard(true);
	...
	global_connectAP.channel=11;
	global_connectAP.flags=0;
	global_wepkeyid=0;
	global_wepmode=0;
	global_dhcp=0;
	global_ipaddr=0x6D01A8C0;
	global_snmask=0x00FFFFFF;
	global_gateway=0x0101A8C0;
	global_dns1=0x0101A8C0;
	global_dns2=0x0101A8C0;
